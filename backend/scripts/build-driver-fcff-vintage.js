...
function comparisonShadow({ fixture, results, candidate, targetIndexes, fixturePath, resultsPath }) {
  ...
  return {
    ...
    ...(candidate.targeting ? { targeting: candidate.targeting } : {}),
    warnings: ["Candidate assumptions are shadow-only; production aggregateDCFInputs and routes are unchanged."],
  };
}

function fixedRosterPathHash(path) {
  if (!path || !existsSync(path)) return null;
  return sha256Bytes(readFileSync(path));
}

function buildFixedRosterShadow({ fixture, results, fixturePath, resultsPath }) {
  if (!fixture || !Array.isArray(fixture.rows)) throw new Error("Fixed-roster shadow requires a fixture with rows");
  if (!results || !Array.isArray(results.records) || !results.summary) throw new Error("Fixed-roster shadow requires baseline results");
  const replay = runValuationBacktest(fixture);
  if (!isDeepStrictEqual({ records: results.records, summary: results.summary }, replay)) {
    throw new Error("Baseline replay metadata or records mismatch");
  }
  const fixtureSha256 = fixedRosterPathHash(fixturePath);
  const resultsSha256 = fixedRosterPathHash(resultsPath);
  if (fixtureSha256 && results.fixtureSha256 && results.fixtureSha256 !== fixtureSha256) {
    throw new Error("Baseline embedded fixture SHA-256 mismatch");
  }
  const candidate = runDriverFCFFBacktest(fixture, { targetTickers: ORDERED_TICKERS });
  ...
  if (JSON.stringify(targetIndexes) !== JSON.stringify([...Array(ORDERED_TICKERS.length).keys()])
    || JSON.stringify(targetTickers) !== JSON.stringify(ORDERED_TICKERS)
    || targetIndexes.length !== ORDERED_TICKERS.length
    || targetIndexes.some((index) => {
      const record = candidate.records[index];
      return record?.status !== "valued"
        || record.projectionMethod !== "driver-fcff"
        || record.projectionYears !== 10
        || record.driverScheduleYears !== 10;
    })) {
    throw new Error(`Fixed-roster candidate must contain valued ordered 10-year driver-fcff rows; got ${targetTickers.join(",")}`);
  }
  const shadow = comparisonShadow({ fixture, results, candidate, targetIndexes, fixturePath: fixturePath || "fixture.json", resultsPath: resultsPath || "baseline.json" });
  return {
    ...shadow,
    model: {
      ...shadow.model,
      modelId: "driver-fcff/v2-fixed-roster-shadow",
      ...(fixtureSha256 ? { fixtureSha256 } : {}),
      ...(resultsSha256 ? { baselineResultsSha256: resultsSha256 } : {}),
    },
    fixture: { ...shadow.fixture, ...(fixtureSha256 ? { sha256: fixtureSha256 } : {}) },
    baseline: {
      ...shadow.baseline,
      ...(resultsSha256 ? { sha256: resultsSha256 } : {}),
      ...(fixtureSha256 ? { fixtureSha256 } : {}),
    },
    targeting: {
...
async function buildVintage(manifest, {
  ...
  const shadow = buildFixedRosterShadow({ fixture, results, fixturePath: `${checked.baselineId}.json`, resultsPath: `${checked.baselineId}-baseline.json` });
  return {
    fixture,
    results,
    shadow,
...
export { ORDERED_TICKERS, buildFixedRosterShadow, buildVintage, main, parseArgs, publishVintageArtifacts, validateManifest, sha256Bytes };
