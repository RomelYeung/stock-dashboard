...
function runDriverFCFFBacktest(input) {
  const rows = normalizeRows(input);
  const defaults = rows.map((row) => evaluateReplayRow(row));
  assertDriverFCFFV2DefaultReplay(input, rows);
  const targetIndexes = driverFCFFTargetIndexes(rows, defaults);
  ...
  return { records, summary: summarizeBacktest(records), driverFCFFTargetIndexes: targetIndexes };
}
